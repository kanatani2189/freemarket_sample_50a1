...
describe User do
  describe '#create' do
    ...
    it "streetがないと登録できないこと" do
        user = build(:user, street: nil)
        user.valid?
        expect(user.errors[:street]).to include("can't be blank")
    end

    it "area_idがないと登録できないこと" do
        user = build(:user, area_id: nil)
        user.valid?
        expect(user.errors[:area_id]).to include("can't be blank")
    end
